...
try {
  ...
  for (const scenario of scenarios) {
    ...
    await page.evaluate(()=>{const w=window;w.Durak.game.state.settings.animations=false;w.Durak.game.state.settings.speed='fast';w.Durak.SPEEDS.fast=0.01;w.Durak.game.refresh();});
    const playerInfo=await page.evaluate(()=>window.Durak.game.state.players.map(p=>({name:p.name,isBot:p.isBot,difficulty:p.difficulty})));
    ...
    while(completed<scenario.rounds&&iterations<12000){
      iterations++;
      const snap=await page.evaluate(()=>{const s=window.Durak.game.state;return{phase:s.phase,inv:window.DurakMultiplayer.debug.invariants()};});
      ...
      if(snap.phase==='end'){completed++;if(completed<scenario.rounds)await page.evaluate(()=>document.querySelector('[data-action="next-round"]').click());}
