...
async function setNumber(page, key, value) {
  const input = page.locator(`#dev-layout-section input[type="number"][data-dev-key="${key}"]`);
...
try {
  ...
  await page.waitForSelector('#dev-layout-section');
  assert.equal(await page.locator('#dev-layout-section').isVisible(), true, 'DEV section should be visible in Options');
  ...
  console.log('DEV layout smoke: PASS');
  await context.close();
} finally {
  await browser.close();
